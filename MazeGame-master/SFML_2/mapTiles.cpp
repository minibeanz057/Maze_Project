...
mapTiles::mapTiles() {};
mapTiles::mapTiles(sf::RectangleShape _shape) {};

void mapTiles::loadPoints(std::string _fileName, sf::RenderWindow& _win) {
    std::ifstream dataStream;
    std::string line;

#pragma region CSV File Reading
    dataStream.open("map.csv");                                   // open file to a data stream for reading
...
#pragma region Create Graph points 
    ...
    {
        addPoint(sf::Color(0, 0, 255), sf::Vector2u(0,0), sf::Vector2f(0, 0));
...
void mapTiles::addPoint(sf::Color clr, sf::Vector2u origin, sf::Vector2f position) 
{
    sf::RectangleShape pt; pt.setFillColor(clr); pt.setOrigin(0, 0); pt.setPosition(sf::Vector2f(position));  
    points.push_back(pt);
};
void mapTiles::setPointColor(int index, sf::Color _color) {/* not implemented yet*/ };
void mapTiles::clearPointList() {/* not implemented yet*/ };

// Go through the points (vector list) and draw the points (circles) on the render surface (using a reference to window - _win)
void mapTiles::drawPoints(sf::RenderWindow &_win) {
